...
<div class="modal-header">
    <h4 class="modal-title" id="myModalLabel1">Modifier une sortie</h4>
    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
    <span aria-hidden="true"><i class="ft-x font-medium-2 text-bold-700"></i></span>
    </button>
</div>
<?php
    $stock=Stock::get($_REQUEST["id"]);
?>
<form action="bar/sortie/modifier" method="post">
    ...
    <div class="modal-body form-row">
        <div class="col-md-12 col-12">
            <div class="form-group mb-2">
                ...
                <select class="form-control select2" name="idelement" required>
                    ...
                    ?>
                        <option value="<?=$boisson->id?>" <?=$boisson->id==$stock->idelement ? 'selected':''?> ><?=$boisson->intitule?></option>
                    <?php
                        }
                    ?>
                </select>
        ...
        <div class="col-md-12 col-12">
            <div class="form-group mb-2">
                ...
                <input type="text" class="form-control" name="intitule" value="<?=$stock->intitule?>" placeholder="Intitulé" >
            </div>
        </div>
        <div class="col-md-12 col-12">
            <div class="form-group mb-2">
                <label for="basic-form-2">Quantite *</label>
                <input type="number" class="form-control" name="quantite" value="<?=$stock->quantite?>" placeholder="Quantite" required>
            </div>
        </div>
        <div class="col-md-12 col-12">
            <div class="form-group mb-2">
                <label for="basic-form-2">Date *</label>
                <input type="date" class="form-control" name="date" placeholder="Date" value="<?=$stock->date?>" required>
            </div>
        </div>
        <div class="col-md-12 col-12">
            <div class="form-group mb-2">
                <label for="basic-form-2">Motif</label>
                <textarea class="form-control" name="motif" ><?=$stock->motif?></textarea>
            </div>
        </div>
    </div>
        ...
        $(this).ajaxSubmit({
            ...
            success: function(data, statusText, xhr) {
                if (data.state == 'success') {
                    search();
                    toastr.success('sortie modifiée avec succès.');
