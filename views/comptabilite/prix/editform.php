...
<div class="modal-header">
    <h4 class="modal-title" id="myModalLabel1">Modifier un prix</h4>
    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
    <span aria-hidden="true"><i class="ft-x font-medium-2 text-bold-700"></i></span>
    </button>
</div>
<?php
    $prix=prix::get($_REQUEST["id"]);
?>
<form action="comptabilite/prix/modifier" method="post">
    <input type="hidden" name="id" value="<?=$_REQUEST["id"]?>">
    <div class="modal-body form-row">
        <div class="col-md-12 col-12">
            <div class="form-group mb-2">
                <label for="basic-form-1">Intitulé *</label>
                <input type="text" class="form-control" name="intitule"  value="<?=$prix->intitule?>" placeholder="Intitulé" required>
            </div>
        </div>
        <div class="col-md-12 col-12">
            <div class="form-group mb-2">
                <label for="basic-form-2">Montant *</label>
                <input type="number" class="form-control" name="prix" value="<?=$prix->prix?>" placeholder="Prix" required>
            </div>
        </div>
        <div class="col-md-12 col-12">
            <div class="form-group mb-2">
                <label for="basic-form-2">Caisse *</label>
                <select class="form-control select2" name="caisse" required>
                <?php
                    foreach (Caisse::q()->execute() as $caisse) {
                ?>
                    <option value="<?=$caisse->id?>" <?=$caisse->id==$prix->idcaisse ? 'selected':''?> ><?=$caisse->intitule?></option>
                <?php
                    }
                ?>
                </select>
        ...
        $(this).ajaxSubmit({
            ...
            success: function(data, statusText, xhr) {
                if (data.state == 'success') {
                    search();
                    toastr.success('Prix modifié avec succès.');
