...
<div class="modal-header">
    <h4 class="modal-title" id="myModalLabel1">Modifier un espace</h4>
    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
    <span aria-hidden="true"><i class="ft-x font-medium-2 text-bold-700"></i></span>
    </button>
</div>
<?php
    $espace=Espace::get($_REQUEST["id"]);
?>
<form action="espace/liste/modifier" method="post">
    <input type="hidden" name="id" value="<?=$_REQUEST["id"]?>">
    <div class="modal-body form-row">
        <div class="col-md-12 col-12">
            <div class="form-group mb-2">
                <label for="basic-form-1">Nom *</label>
                <input type="text" class="form-control" name="intitule"  value="<?=$espace->intitule?>" placeholder="Nom" required>
            </div>
        </div>
        <div class="col-md-12 col-12">
            <div class="form-group mb-2">
                <label for="basic-form-1">Prix de location</label>
                <input type="number" class="form-control" name="prix"  value="<?=$espace->prix?>" placeholder="Prix de location" >
            </div>
        </div>
    </div>
